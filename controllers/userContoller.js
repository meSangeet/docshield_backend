...
exports.createMultipleStudents = async (req, res, next) => {
  try {
    ...
    let numofstud= await Student.countDocuments({});

    for (let student of students) {
      student.enrollmentNumber = (numofstud+1).toString();
      numofstud++;
    }
  ...
  if (req.body.teachers === "all") {
    try {
      ...
      users.forEach(async (user) => {
        ...
        await sendMail({
          to: user.email,
          subject: "Your credentials have been reset by admin",
          text: `DocShield! admin has decided to reset credentials\n\nYour password is now: ${initialPassword}\n\nPlease keep your credentials safe and change your password upon first login.\nYou can download your digital certificate after it`,
        });
      });
      console.log(user);
      res.status(200).json({ message: "Passwords reset successfully for all teachers." });
    } catch (err) {
      console.error(err);
      res.status(500).json({ message: "Internal server error." });
    }
    ...
    try {
      ...
      const users = await User.find({ role: "teacher", regno: { $in: teachers } });
      users.forEach(async (user) => {
        const initialPassword = Math.random().toString(36).substring(2, 10);
        const hashedPassword = await bcrypt.hash(initialPassword, 12);
        user.password = hashedPassword;
      ...
      res.status(200).json({ message: "Passwords reset successfully for selected teachers." });
    } catch (err) {
      console.error(err);
      res.status(500).json({ message: "Internal server error." });
    }
